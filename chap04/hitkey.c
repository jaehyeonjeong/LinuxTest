...
int main(int argc, char** argv[1])
{
	...
	for(i = 0; ; i++){
		...
		printf("%20d\t\t\r", i);				/*현재 카운터한 숫자를 출력한다. '\r'은 한줄에서만 표시(캐리지 리턴)*/
		usleep(1000000);						/*1000000밀리초(1초) 동안 잠시 쉰다.*/
	}
END:
	printf("Good Bye!\n");
	return 0;
...
//시스템과의 대화를 위해 사용되는 디바이스로 /dev/tty라는 디바이스 파일을 사용한다.
/*터미널의 기본 제어에는 termios 구조체와 관련 함수들을 사용할 수 있는데, 위에서 입력되는 문자를
  화면에 출력할 수 없도록 터미널 제어 함수인 tcgetattr()함수와 tcsetattr()함수를 이용해서 터미널
  에 대한 값을 가져오고나 설정한다. 터미널을 제어하기 전에 tcgetattr() 함수를 이용해서 현재의 속성을 저장한다.*/
