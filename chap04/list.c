//디렉터리에 대한 정보를 출력하는 코드를 작성 ls -RI와 유사
#include <unistd.h>
#include <stdio.h>
#include <string.h>
#include <fcntl.h>
...
int listDir(char* arg)
{
	...
	printf("\n%s: Directory\n", arg);
	//디렉터리를 순환해서 표시하려면 재귀 호출이 필요하다.
	//디렉터리에 대한 내용을 읽기 위해 디렉터리를 열고 해당 디렉터리로 이동한 후 절대 경로를 화면에 표시한다.
	while((dirt = readdir(pdir)) != NULL){	/*현재 디렉터리를 읽을 수 있으면 순환*/
		lstat(dirt->d_name, &statBuf);
		/*파일의 종류를 검사*/
		if(S_ISDIR(statBuf.st_mode))		/*파일의 이름과 해당 파일에 대한 정보를 가지고 있는 곳을 가리키는 포인터*/
			permission[0] = 'd';
		else if(S_ISLNK(statBuf.st_mode))	/*다른 파일을 가리키는 포인터 역할을 하는 파일*/
			permission[0] = 'l';
		else if(S_ISCHR(statBuf.st_mode))	/*문자 디바이스를 가리키는 파일로 입출력 수행 시 임의의 크기의 바이트 열 전송*/
			permission[0] = 'c';
		else if(S_ISBLK(statBuf.st_mode))	/*블록 디바이스를 가리키는 파잉ㄹ로 입출력 수행 시 일정한 크기로 데이터 전송*/
			permission[0] = 'b';
		else if(S_ISSOCK(statBuf.st_mode))	/*네트워크 통신에 사용되는 파일*/
			permission[0] = 's';
		else if(S_ISFIFO(statBuf.st_mode))	/*프로세스 간 통신(IPC)에 사용되는 파일(named pipe)*/
			permission[0] = 'P';
		else								/*기타 정규 파일*/
			permission[0] = '-';
		...
		//ls -l 명령어를 실행했을 때 표시되는 앞의 10개 문자열과 같은 내용을 저장한다.
	...
	for(i = 0; i < count; i++){					/*다른 디렉터리에 대한 재귀 호출*/
		if(listDir(dirName[1]) == -1) break;
	}

	printf("\n");				
	closedir(pdir);					/*열었던 디렉터리의 스트림을 닫는다.*/
	chdir(".."); 					/*원래 디렉터리로 이동*/
	return 0;
}
